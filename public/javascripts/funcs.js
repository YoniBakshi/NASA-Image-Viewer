...
(function () {
    ...
    const APIKEY = 'JbfLgvG7Tu218jEiWKZH4jMOrscQvu1mDJoctND3';

    document.addEventListener("DOMContentLoaded", function () {
        document.getElementById("uname").addEventListener("keyup", validatorName)
        //get a picked date
        ...
        //get at first the 3 last days pictures by default when login
        document.getElementById("loginBox").addEventListener("submit", (elm) => {
            elm.preventDefault()
            if (validatorName()) {
                date.initDate()
                getDataFromNASA()
        ...
        //check written comment if valid (between 1 - 128 characters) and add it to comment's array.
    ...
    const date = function () {
        ...
        /**
         * Module parameters.
         * NOTE : to match the requested format of timeline - we'll switch the meanings :
    ...
    const create3ImgSerie = (data) => {
        ...
        readMoreLessBtn()       // If needed - AddEventListener to "Read more/less" explanation field.
    }
    ...
    const createHtmlImgInfo = (currData) => {
        ...
        if(currData['explanation'].length > 100) {
            start = currData['explanation'].split(' ').slice(0,40).join(' ')
            end = currData['explanation'].split(' ').slice(40,).join(' ')
            moreBtn = `${start}<a class="btn btn-link readMore" id="b${currData.date}m">...Read more</a> <span class="d-none" id="b${currData.date}e"> ${end}</span>
    ...
    function getDataPostedComments() {
        ...
        fetch(`/resources/${date.getNewDate().date}`)
            .then(function(response) {
    ...
    const addCommentsToModal = (data) =>{
        ...
        data.forEach(function(item) {
            let delLink = item.username === document.getElementById("uname").value ?
                ` <a class="btn btn-link mr-2 to-del" href="del/${item.postId}" id="del-${item.postId}"> del </a>` : ``
            commentInfo +=
                `<div class="card mb-4">
                        <div class="card-body">
    ...
    function delPost(event) {
        ...
        fetch(`/del/${deleteItem}`, {method: 'DELETE'})
            .then(function(response) {
                return response.json();
            }).then(function(data) {
            if(data.status >= 400)
