...
(function () {
    ...
    const date = function () {
        ...
        function initDate() {
            creatToggle("header-container");      //Display title container with pick a date box
    ...
    /**
     * Validate entered username - must contain digits and letters only, length must be between 1-24.
     * @returns {boolean} True if username is valid, False if not valid
     */
    function validatorName() {
        const inpUname = document.getElementById("uname");
        if (!(/^[A-Za-z0-9]{1,24}$/.test(inpUname.value.trim()))) {
